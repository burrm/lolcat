...
spc.chart.variables.individual.and.movingrange.exponential.simple <- function(
    individuals
...
#    print(argg)
rate <- 1/chart1.center.line

if (is.na(chart1.control.limits.ucl) | is.na(chart1.control.limits.lcl)) {
    tmp <- natural.tolerance.exp.simple(rate=rate)
...
if (is.na(chart1.zone.ab.upper) | is.na(chart1.zone.ab.lower)) {
    tmp <- natural.tolerance.exp.simple(rate=rate, total.area = 2*pnorm(-2))
...
if (is.na(chart1.zone.bc.upper) | is.na(chart1.zone.bc.lower)) {
    tmp <- natural.tolerance.exp.simple(rate=rate, total.area = 2*pnorm(-1))
